...
let DisplayProducts = () => {
    ...
    records.forEach((rec) => {
        str += `
        <div class="col-11 col-sm-10 col-md-6 col-lg-4 col-xl-3 d-l-none d-sm-block py-4 ">
            <div  class="card p-3 shadow product-card rounded-4  bg-blur">
            <div class="card-body p-0" onclick="return CartMenu(${rec.id})">
                <img src="${rec.thumbnail}"  class="card-img-top img-fluid" alt="Product Image" ">
            <div class="d-flex align-items-center">
                <span>
                    <h6 class="card-title text-wrap" style="">${rec.title}</h6>
                    <p class="card-text text-black mt-2">${rec.description}</p>
                    <p class="card-text text-black fw-bold mb-2">₹ ${rec.price}</p>
                </span>
            </div>
        </div>
        <div class="d-flex justify-content-between">
        <button class="btn btn-primary" onclick="return AddToCart(${rec.id})">Add to Cart</button>
        <button class="btn text-black btn-success">Buy Now</button>
        </div>
        </div>
        </div>
...
let AddToCart = (id) => {
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    let Productcart = cart.find(e => e.id == id);
    let singlepro = Product.find(prd => prd.id == id);
    ...
    if (Productcart) {
        ...
        let data = {
            ...
            rating: singlepro.rating,
            cate: singlepro.category,
            brand:singlepro.brand,
            wrnty:singlepro.warrantyInformation,
...
let model = document.querySelector('.menu');
...
let CartMenu = (id) => {
    ...
    let rec = items.find(ele => ele.id == id);

    if (rec) {
        print = `
            <img src="${rec.thumbnail}" alt="img">
        `;
    }
    img.innerHTML = print;
    if (rec) {
        str = `
            <h5 class="text-black pb-2">Title : ${rec.title}</h5>
            <p class="text-black fw-bold"><span class="fw-normal">Price :</span> ₹ ${rec.price}</p>
            <p class="text-black">Rating : <i class="fa-sharp fa-solid fa-star"></i>${rec.rating}</p> 
            <p class="text-black">Category : ${rec.category}</p>
            <p class="text-black">Brand : ${rec.brand}</p>
            <p class="text-black">Description : ${rec.description}</p>
            <div class="d-flex">
                <button class="mx-1 btn btn-primary" onclick="return AddToCart(${rec.id})">Add to Cart</button>
                <button class="mx-1 btn text-black btn-success">Buy Now</button>
            </div>
        `
    }
    menubody.innerHTML = str;
